...
if (!args[0]) return message.channel.send(new MessageEmbed().setDescription(`${message.author} <a:crs:797920746675896341> Hata! Komutu çalıştırmak için \`aç\` veya \`kapat\` demen gerekiyor.`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} <a:blob_tick:797837802599219250> Reklam filtersini başarıyla \`açtın\`.`).setColor('0x348f36').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
}
if (args[0] == 'kapat') {
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} <a:blob_tick:797837802599219250> Reklam filtersini başarıyla \`kapattın\`.`).setColor('0x348f36').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
}
};
